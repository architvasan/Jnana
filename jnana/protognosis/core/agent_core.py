...
def _extract_target_sequence(research_goal: str) -> str:
        """
...
class ResearchRecommendation:
    ...
    def __init__(self, 
                 ...
                 metadata: Optional[Dict] = None):
        """
        Initialize a research recommendation.
        
        Args:
            content: The full text of the hypothesis
            summary: A brief summary of the hypothesis
            agent_id: ID of the agent that generated this hypothesis
            recommendation_id: Optional ID for the hypothesis (auto-generated if not provided)
            elo_rating: Initial Elo rating (default: 1200)
            metadata: Optional additional information about the hypothesis
        """
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> 'ResearchRecommendation':
        """Create a hypothesis from a dictionary."""
        recommendation = cls(
            content=data["content"],
            summary=data["summary"],
    ...
    def __str__(self) -> str:
        return f"Recommendation {self.recommendation_id} - {self.summary} (Elo: {self.elo_rating:.1f})"
...
class ContextMemory:
    ...
    def load(self):
        ...
        try:
            ...
            for r_data in data.get('recommendations', []):
                recommendation = ResearchRecommendation.from_dict(r_data)
    ...
    def add_recommendation(self, recommendation: ResearchHypothesis) -> None:
        """
    ...
    def get_recommendation(self, recommendation_id):
        ...
        Returns:
            recommendatino if found, None otherwise
        """
    ...
    def append_to_history(self, results: list[str]|str, num_history: int|None = None) -> None:
        """
        Append to history
        ...
        if isinstance(results, list):
            self.metadata["history"].extend(results)
        else:
            self.metadata["history"].append(results)
        if num_history is not None:
            self.metadata["num_history"] = num_history
